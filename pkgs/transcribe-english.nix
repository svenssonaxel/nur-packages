# English audio transcription via faster-whisper (Whisper large-v3), CPU/float32.
...
in
pkgs.writeShellScriptBin "transcribe-english" ''
  set -euo pipefail
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      -o|--outdir)
        ...
        ;;
      -h|--help)
        echo "Usage: transcribe-english [-o/--outdir DIR] file1 [file2...]"
        echo ""
        echo "Transcribes English audio using Whisper large-v3 (high-quality model)."
        echo ""
        echo "Options:"
        ...
        echo ""
        echo "Examples:"
        echo "  transcribe-english recording.wav"
        echo "  transcribe-english -o ~/transcripts *.wav"
        echo "  transcribe-english file1.mp3 file2.mp3"
        echo ""
  ...
  if [ ''${#FILES[@]} -eq 0 ]; then
    ...
    echo "Usage: transcribe-english [-o/--outdir DIR] file1 [file2...]" >&2
    exit 1
  fi
  ...
  for FILE in "''${FILES[@]}"; do
    ...
    echo "   Model: Whisper large-v3 (float32)"
    ...
    nice -n 19 ${python}/bin/python3 ${./transcribe-english.py}

...
# Auto-discovers every `pkgs/*.nix` (except this file and checks.nix) and imports
# it with `{ inherit pkgs; }`, so adding a package needs no edit here. Each
# `pkgs/<name>.nix` is `{ pkgs }:` (extra args allowed) returning a derivation.
